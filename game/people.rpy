...
screen people_nav():
    ...
    viewport:
        ...
        vbox:
            spacing 10
            xoffset 350
            textbutton "print()" action ShowMenu("zack")
            textbutton "Python's Background" action ShowMenu("nelson")

            ##The following button will require a condition to be true in order to appear.
            if persistent.bob:
                textbutton "Bob" action ShowMenu("bob")

            textbutton "Variable Name" action NullAction()
            textbutton "Python Syntax" action NullAction()
            textbutton "Python Indention" action NullAction()
            textbutton "Variables and Comments" action NullAction()
